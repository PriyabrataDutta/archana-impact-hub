...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-background/95 backdrop-blur-md shadow-sm" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between h-24 px-5 lg:px-12">
        <a href="#" className="flex items-center gap-3">
          <img
            src={logoIcon}
            alt="Archana Foundation"
            className={`h-16 w-auto object-contain transition-all drop-shadow-md ${scrolled ? "" : "brightness-0 invert drop-shadow-lg"}`}
          />
          <span className={`font-display text-xl md:text-2xl tracking-tight transition-colors font-semibold ${scrolled ? "text-foreground" : "text-hero-foreground drop-shadow-md"}`}>
            Archana Foundation
          </span>
        </a>
